...
export default function LoginPage() {
  ...
  return (
    <form onSubmit={handleLogin} className="max-w-sm mx-auto mt-20 flex flex-col gap-4">
      ...
      <button className="bg-blue-600 text-white p-2" type="submit">{t('login.submit')}</button>
      <button type="button" onClick={google} className="bg-red-500 text-white p-2">{t('login.google')}</button>
    </form>
  );
}
